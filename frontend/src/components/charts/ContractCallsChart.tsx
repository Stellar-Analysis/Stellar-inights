...
import type { SorobanContractCallPoint } from "@/lib/soroban-api";

interface ContractCallsChartProps {
  data: SorobanContractCallPoint[];
  loading?: boolean;
}
...
export function ContractCallsChart({
  ...
  if (chartData.length === 0) {
    return (
      <div className="glass-card rounded-2xl p-6 border border-border/50 flex flex-col items-center justify-center h-[420px]">
        ...
        <p className="text-sm font-mono text-muted-foreground uppercase tracking-widest text-center max-w-md">
          No contract-call time series yet. Data appears once{" "}
          <code className="text-accent">/api/v1/soroban/contract-calls</code>{" "}
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between mb-8 gap-4">
        <div className="flex-1">
          ...
          <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">
            Daily event counts from contract_events // not distinct txs
          </p>
        </div>
        <ChartExportButton chartRef={chartRef} chartName="Contract Calls" />
      </div>
